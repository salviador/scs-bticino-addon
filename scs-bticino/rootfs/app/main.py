...
# Leggi porta seriale da variabile d'ambiente
...
async def Node_Red_flow(jqueqe):
	#logger("START -> Node_Red_flow")
	while(True):
		#logger("WAIT -> Node_Red_flow")
		v = await jqueqe.get()
		n = nodered.nodered()
		n.main()
...
def popula_device():
	...
	for item in allDevice:
		...
		if (item['tipo_attuatore'] == "on_off"):
			...
			#logger("--- DEVICE serrande_tapparelle------------------")
			...
			#logger("--- DEVICE sensori_temperatura------------------")
			...
			#logger("--- DEVICE CAMPANELLO PORTA------------------")
...
async def tsk_refresh_database(jqueqe):
	...
	while(True):
		try:
			v = await jqueqe.get()
			#logger(f'{time.ctime()} *NEED Refresh Database* ' f'{v}')
			#logger(type(v))
			...
			logger("2 - ERRRRRRRRRRRRRRRRRRRRR")
			logger(e)		
...
async def mqtt_action(jqueqe):
	while(True):
		try:
			...
			#logger(f'{time.ctime()} *MQTT topic_filter* ' f'{topic} '  f'{payload} ')
					...
					if(ndevice == device_name):
						...
						if(tdevice.name == SCS.TYPE_INTERfACCIA.on_off.name):
							...
							if(action.lower() == "percentuale"):
								#logger("3 --- SERANDA MQTT AZIONE in %: ", message)

								try:
									cmd1 = loop.create_task(device.Azione(int(message), lock_uartTX))
									await (cmd1)
								except Exception as e:
									logger("EEEEEEEEEEEERRRRRROTTTTT")
									logger(e)
			...
			logger("3 - ERRRRRRRRRRRRRRRRRRRRR")
			logger(e)
...
async def deviceReceiver_from_SCSbus(jqueqe):
	while True:
		...
		try:
			#logger("uart_rx_deviceReceiver LEN: {}" .format(len(trama)))
			#logger("uart_rx_deviceReceiver: {}" .format(trama))
			...
			logger("4 - ERRRRRRRRRRRRRRRRRRRRR")
			logger(e)
...
async def start_tornado(jqueqe, jqueqeNodeRed):
	logger(f'{time.ctime()} WEB SERVER start')
	...
	webapp.tornado.platform.asyncio.AsyncIOMainLoop().install()
	#logger(f'{time.ctime()} EXIT WEB SERVER')
